...
사용자의 발화에서 핵심 감정(두려움, 불안, 외로움 등)과 상황(취업, 가족 갈등 등), 주제/교리 및 일상적 궁금증을 파악.
중복되는 경우(예: ‘불안 + 결정 필요 + 믿음/사랑’)가 있으면 우선순위를 두거나 복합적으로 처리.
태그 매칭
...
위와 같은 프로세스를 통해, 사용자의 질문 → 감정/상황/주제 및 일상 분류 → 관련 성경 구절 매칭 →
